...
with col_main:
    ...
    for _, row in df.iterrows():
        ...
        if is_valid(row.get('rsi')): rsi_line.append({'time': t, 'value': row['rsi']})
        if is_valid(row.get('obv')): obv_line.append({'time': t, 'value': row['obv']})
        if is_valid(row.get('bias')): bias_line.append({'time': t, 'value': row['bias']})

    # --- 圖表配置 ---
    # 【關鍵設定 1】: handleScroll 開啟，允許拖曳不還原
    common_opts = {
    ...
    # --- 2. 副圖 (隱形面板技術) ---
    # 我們 "永遠" 產生這些面板，但如果沒勾選，就把高度設為 0 (或極小值)，並隱藏內容
    format_2f = {"type": "price", "precision": 2, "minMove": 0.01}
    
    # 輔助函數：生成副圖配置
    def add_sub_chart(is_shown, series_list, height=120):
        # 如果不顯示，高度設為 0.001 (不能是 0，有些瀏覽器會怪怪的)，並隱藏所有線條
        pane_height = height if is_shown else 0.001 
        for s in series_list:
            # 強制覆蓋 visible 屬性
            if "options" not in s: s["options"] = {}
            s["options"]["visible"] = is_shown
            
        panes.append({"chart": common_opts, "series": series_list, "height": pane_height})

    # VOL
    add_sub_chart(show_vol, [{"type": "Histogram", "data": vols, "options": {"priceFormat": {"type": "volume"}, "title": "VOL"}}], 120)
    ...
    add_sub_chart(show_macd, s_macd, 150) # MACD 高一點
    ...
    add_sub_chart(show_kdj, s_kdj, 120)

    # RSI
    add_sub_chart(show_rsi, [{"type": "Line", "data": rsi_line, "options": {"color": "#E040FB", "title": "RSI", "priceFormat": format_2f}}], 120)

    # OBV
    add_sub_chart(show_obv, [{"type": "Line", "data": obv_line, "options": {"color": "#FFA500", "title": "OBV", "priceFormat": {"type": "volume"}}}], 120)

    # BIAS
    add_sub_chart(show_bias, [{"type": "Line", "data": bias_line, "options": {"color": "#607D8B", "title": "BIAS", "priceFormat": format_2f}}], 120)

    # --- 【關鍵修改】Key 永遠不變 ---
    # 移除了所有 show_xxx 變數，只保留資料源變數
    # 這樣 Streamlit 就永遠不會 "重置" 圖表，只會更新屬性 (Visible/Height)
    st_key = f"desk_v90_stealth_{ticker}_{interval_label}_{start_date}_{end_date}"
